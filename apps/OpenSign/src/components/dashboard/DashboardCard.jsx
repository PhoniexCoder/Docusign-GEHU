...
const DashboardCard = (props) => {
  ...
  return (
    <div
      onClick={() => openReport()}
      className={`relative h-full flex flex-col justify-center bg-base-100 p-5 rounded-2xl shadow-sm border border-base-200 hover:shadow-md transition-all duration-200 ${props.Data && props.Data.Redirect_type
        ? "cursor-pointer"
        : "cursor-default"
        }`}
    >
      <div className="flex items-center justify-start gap-5">
        <div className={`rounded-2xl w-[64px] h-[64px] flex justify-center items-center shrink-0 shadow-lg shadow-base-200 opensigndark:shadow-primary/10 ${props.BgColor?.includes("bg-primary") ? "bg-primary/10 text-primary opensigndark:bg-gradient-primary opensigndark:text-white" :
          props.BgColor?.includes("bg-info") ? "bg-info/10 text-info opensigndark:bg-gradient-info opensigndark:text-white" :
            props.BgColor?.includes("bg-success") ? "bg-success/10 text-success opensigndark:bg-gradient-success opensigndark:text-white" :
              props.BgColor?.includes("bg-warning") ? "bg-warning/10 text-warning opensigndark:bg-gradient-warning opensigndark:text-white" : "bg-primary/10 text-primary opensigndark:bg-gradient-primary opensigndark:text-white"
          }`}>
          <i
        ...
        <div className="flex flex-col">
          <div className="text-sm font-medium text-base-content/60 mb-0.5">
            {t(`dashboard-card.${props.Label}`)}
          </div>
          <div className="text-3xl font-bold text-base-content font-outfit">
            {loading ? <div className="loader-01"></div> : setFormat(response)}
          </div>
        </div>
      </div>
      <div className="absolute top-4 right-4 text-base-content/30 hover:text-base-content/60 transition-colors">
        <Tooltip
          id={props.Label}
