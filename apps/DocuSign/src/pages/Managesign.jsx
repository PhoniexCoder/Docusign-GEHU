          ...
          <div className="flex flex-col md:flex-row gap-2 md:gap-5">
              ...
              <UploadThingButton
                ...
                onUploadComplete={(res) => {
                  if (res?.[0]) {
                    ...
                    setSignature(""); // clear canvas data
                    setImage(res[0].url);
                    setIsValue(true);
                    // Track usage
                    const tenantId = localStorage.getItem("TenantId");
                    const userId = Parse?.User?.current()?.id;
                    SaveFileSize(res[0].size, res[0].url, tenantId, userId);
                  }
                }}
              />
              <div>
                ...
                <div className="flex flex-row justify-between w-full">
                  ...
                  <div className="flex flex-row gap-2 text-sm pr-2">
                    <div
                      ...
                      onClick={() => { }}
                    >
                      {/* Upload handling now done via button above, or re-layout needed if we want text link trigger */}
                      {/* Re-using UploadThingButton structure implies button UI. The existing UI uses text links 'Upload' */}
                      {/* For now, I will comment out the text link trigger logic or leave it as no-op since button is visible */}
                      {t("upload")}
                    </div>
                    <div
                      type="button"
            ...
            <div>
              ...
              <UploadThingButton
                ...
                onUploadComplete={(res) => {
                  if (res?.[0]) {
                    if (initailsRef.current) initailsRef.current.clear();
                    setInitials("");
                    setImgInitials(res[0].url);
                    if (image || signature) setIsValue(true);
                    const tenantId = localStorage.getItem("TenantId");
                    const userId = Parse?.User?.current()?.id;
                    SaveFileSize(res[0].size, res[0].url, tenantId, userId);
                  }
            ...
            <div>
              ...
              <div
                ...
                className="mystampCanvas docusigndark:bg-[#1f2937] cursor-pointer border-[2px] docusigncss:border-[#888] docusigndark:border-[#4b5563] rounded-box flex flex-col overflow-hidden w-full h-full aspect-[5/2] justify-center items-center mr-2"
              >
                {stamp ? (
                  <img
                    alt="signature"
