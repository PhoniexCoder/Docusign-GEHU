...
const Forms = (props) => {
  ...
  const handleFileInput = withSessionValidation(async (e) => {
    ...
    try {
      ...
      // Use UploadThing to upload the merged PDF
      const pdfFile = new File([pdfBytes], pdfName, { type: "application/pdf" });
      const utRes = await startUpload([pdfFile]);

      if (utRes && utRes[0]) {
        uploadedUrl = utRes[0].url;
      }
      if (uploadedUrl) {
  ...
  return (
    <div
      className={`${isSubmit ? "" : props.title === "Sign Yourself" ? "w-full max-w-4xl mx-auto" : "shadow-md rounded-box my-[2px] p-3 bg-base-100 text-base-content"}`}
    >
      ...
      {isSubmit ? (
        ...
        <>
          ...
          <form onSubmit={handleSubmit}>
            {props.title === "Sign Yourself" ? (
              <div className="op-card bg-base-100 shadow-xl overflow-hidden">
                <div className="op-card-body p-8">
                  <h2 className="op-card-title text-2xl font-bold mb-1">{t(props?.title)}</h2>
                  ...
                  {fileload && (
                    <div className="flex items-center gap-x-2 mb-4">
                      <div className="h-2 rounded-full w-full bg-base-200">
                        <div
                  ...
                  <div className="op-form-control w-full mb-6">
                    <label className="op-label">
                      <span className="op-label-text font-medium text-base">
                    ...
                    {fileupload.length > 0 ? (
                      <div className="flex items-center gap-3 p-4 border border-base-300 rounded-lg bg-base-50/50">
                        <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary">
                          <i className="fa-duotone fa-file-pdf text-xl"></i>
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="font-medium truncate text-sm">{selectedFiles.join(", ")}</p>
                          <p className="text-xs text-success font-medium flex items-center gap-1">
                            <i className="fa-solid fa-circle-check text-[10px]"></i> {t("upload-success")}
                          </p>
                        </div>
                        <button
                          type="button"
                          ...
                          className="op-btn op-btn-ghost op-btn-circle op-btn-sm text-error hover:bg-error/10"
                        >
                      ...
                      <div
                        className="border-2 border-dashed border-base-300 rounded-xl p-8 text-center hover:border-primary hover:bg-base-200/30 transition-all cursor-pointer group"
                        onDrop={handleDrop}
                      ...
                      >
                        ...
                        />
                        <div className="w-16 h-16 rounded-full bg-base-200 flex items-center justify-center mx-auto mb-4 group-hover:bg-primary/10 group-hover:text-primary transition-colors">
                          <i className="fa-duotone fa-cloud-arrow-up text-3xl"></i>
                        </div>
                        <h3 className="font-semibold text-lg mb-1 group-hover:text-primary transition-colors">{t("drag-drop-file")}</h3>
                        <p className="text-sm text-base-content/60">{t("or-click-browse")}</p>
                        <p className="text-xs text-base-content/40 mt-2">{t("supported-formats")}</p>
                      </div>
                    )}
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-2">
                    <div className="op-form-control w-full">
                      <label className="op-label">
                        <span className="op-label-text font-medium">{t("document-title")}
                          <span className="text-error ml-1">*</span>
                        </span>
                      </label>
                      <input
                        type="text"
                        name="Name"
                        className="op-input op-input-bordered w-full focus:outline-none focus:border-primary transition-colors"
                        value={formData.Name}
                        onChange={(e) => handleStrInput(e)}
                    ...
                    <div className="op-form-control w-full">
                      <label className="op-label">
                        <span className="op-label-text font-medium">{t("note")}
                          <span className="text-error ml-1">*</span>
                        </span>
                      </label>
                      <input
                  ...
                  {isAdvanceOpt && (
                    <div className="bg-base-200/50 p-4 rounded-lg mb-6">
                      <div className="text-sm font-semibold mb-3">{t("advanced-options")}</div>
                      <div className="op-form-control w-full my-2">
                        <label className="op-label cursor-pointer justify-start gap-3">
                          <input
                            type="checkbox"
                            className="op-checkbox op-checkbox-primary op-checkbox-sm"
                            checked={formData.NotifyOnSignatures}
                            onChange={(e) => handleNotifySignChange(e.target.checked)}
                          />
                          <span className="op-label-text">{t("notify-on-signatures")}</span>
                        </label>
                      </div>
                    </div>
                  )}

                  <div className="flex flex-col-reverse sm:flex-row items-center justify-between gap-4 mt-2 pt-6 border-t border-base-200">
                    <button
                      className="op-btn op-btn-ghost text-base-content/70 hover:text-base-content normal-case"
                      onClick={() => handleCancel()}
                      type="button"
                    >
                      <i className="fa-regular fa-arrow-left mr-2"></i> {t("back-to-home")}
                    </button>
                    ...
                    <div className="flex items-center gap-4 w-full sm:w-auto">
                      ...
                      >
                        {isAdvanceOpt ? t("hide-advanced-options") : t("advanced-options")}
                      </span>
                      <button
                        className={`op-btn op-btn-primary px-8 normal-case ${isSubmit || !fileupload ? "loading disabled" : ""}`}
                        type="submit"
                        disabled={isSubmit || !fileupload}
                      >
                        {isSubmit ? t("processing") : t("next")} <i className={`fa-regular fa-arrow-right ml-2 ${isSubmit ? "hidden" : ""}`}></i>
                      </button>
                    </div>
                  </div>
                </div>
            ...
            {!isSubmit && props.title !== "Sign Yourself" && (
              <>
                {fileload && (
                ...
                {props.title !== "Sign Yourself" && (
                  <div className="text-xs mt-3">
                    ...
                    <textarea
                      type="text"
                      name="Description"
                      className="op-textarea op-textarea-bordered op-textarea-sm focus:outline-none hover:border-base-content w-full text-xs"
                ...
                {props.title !== "Sign Yourself" && (
                  <div className="text-xs mt-3">
                    <label className="block">
                      {t("note")}
                      <a data-tooltip-id="note-tooltip" className="ml-1">
                        <sup>
                          <i className="fa-light fa-question rounded-full border-[#33bbff] text-[#33bbff] text-[13px] border-[1px] py-[1.5px] px-[4px]"></i>
                        </sup>
                      </a>
                      <Tooltip id="note-tooltip" className="z-[999]">
                ...
                <div className="mt-3 mb-3">
                  <SelectFolder folder={folder} handleFolder={handleFolder} />
                </div>

                <div className="flex border-[1px] rounded-[10px] bg-base-100 p-3 h-auto md:h-[300px] flex-col md:flex-row">
