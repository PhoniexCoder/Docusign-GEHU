...
export const embedDocId = async (pdfOriginalWH, pdfDoc, documentId) => {
  const appName = "GEHUDOCSI";
  // `fontBytes` is used to embed custom font in pdf
  const fontBytes = await fileasbytes(
    "https://cdn.DocuSignlabs.com/webfonts/times.ttf"
  );
...
export const embedWidgetsToDoc = async (
  ...
  if (!hasError) {
    ...
    return {
      error:
        "This pdf is not compatible with DocuSign please contact <support@DocuSignlabs.com>"
    };
  }
};
...
export const getAppLogo = async () => {
  ...
  try {
    ...
    if (tenant) {
      localStorage.setItem("appname", "DocuSign");
    ...
    if (err?.message?.includes("valid JSON")) {
      ...
      return { logo: appInfo.applogo, user: "error", error: err.message };
    }
  }
};
...
export const handleToPrint = async (event, setIsDownloading, pdfDetails) => {
  ...
  try {
    ...
    //if we give this `appInfo.baseUrl` as a base url then in public-profile it will create base url of it's window.location.origin ex- DocuSign.me which is not base url
    const axiosRes = await axios.post(
      `${localStorage.getItem("baseUrl")}/functions/getsignedurl`,
      {
        url: pdfUrl,
